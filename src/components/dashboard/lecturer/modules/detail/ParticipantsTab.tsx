...
import { AlertTriangle, Download, Mail, UserMinus } from "lucide-react";
...
export function ParticipantsTab() {
  ...
  const confirmKick = () => {
    ...
    setPeserta((currentPeserta) =>
      currentPeserta.filter((item) => item.id !== kickTarget.id)
    );
    setKickTarget(null);
    setReason("");
  };
  ...
  const handleEmail = (nim: string) => {
    const mailtoUrl = `mailto:student_${nim}@edubidan.id?subject=Informasi%20Modul%20Pembelajaran`;
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <button
          type="button"
          className="w-full sm:w-auto px-4 py-2.5 rounded-xl border border-border text-sm font-bold text-foreground hover:bg-muted transition-colors flex items-center justify-center gap-2 shadow-sm hover:shadow-md"
        >
          ...
          <div className="bg-card rounded-2xl sm:rounded-3xl border border-border w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-6 sm:p-8">
              ...
              <p className="text-xs sm:text-sm text-muted-foreground font-medium leading-relaxed mb-6">
                ...
                (NPM {kickTarget.nim}) dari modul ini. Mahasiswa akan kehilangan
                akses ke materi dan progresnya akan dihapus permanen.
              </p>
